...
export default class Gallery extends React.Component {
  constructor(props) {
    ...
    this.state = {
      ...
      prev: <span className="glyphicon glyphicon-chevron-left"/>,
  ...
  render() {
    const {
      isOpen, index, images, profile, photoIndex, repeatIcon, hideIcon, prev, next
    } = this.state;
    return (
      <Carousel
        interval={null}
        indicators={false}
        activeIndex={index}
        prevIcon={index === 0 ? hideIcon: prev}
        nextIcon={index === 2 ? repeatIcon : next}
